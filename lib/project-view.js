...
export default class ProjectView extends FuzzyFinderView {
  constructor (paths) {
    ...
    const shouldReIndexOnFocus = atom.config.get('jf-fuzzy-finder.reIndexOnFocus') === false ? false : true

    if (shouldReIndexOnFocus) window.addEventListener('focus', windowFocused)

    this.disposables.add(new Disposable(() => { window.removeEventListener('focus', windowFocused) }))

    this.disposables.add(atom.project.onDidAddBuffer(() => { this.reloadPaths = true }))
    this.disposables.add(atom.config.onDidChange('jf-fuzzy-finder.ignoredNames', () => { this.reloadPaths = true }))
