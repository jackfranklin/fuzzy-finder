...
export default class FuzzyFinderView {
  constructor () {
    ...
    this.selectListView = new SelectListView({
      ...
      didConfirmSelection: (item) => {
        this.confirm(item, {searchAllPanes: atom.config.get('jf-fuzzy-finder.searchAllPanes')})
      },
      ...
      elementForItem: ({filePath, projectRelativePath}) => {
        ...
        const iconClasses = FileIcons.getService().iconClassForPath(filePath, 'jf-fuzzy-finder')
        let classList
    ...
    this.selectListView.element.classList.add('jf-fuzzy-finder')
    ...
    atom.commands.add(this.selectListView.element, {
      ...
      'jf-fuzzy-finder:invert-confirm': () => {
        this.confirm(
          this.selectListView.getSelectedItem(),
          {searchAllPanes: !atom.config.get('jf-fuzzy-finder.searchAllPanes')}
        )
      }
    })
    ...
    this.subscriptions.add(
      atom.config.observe('jf-fuzzy-finder.useAlternateScoring', (newValue) => {
  ...
  cancel () {
    if (atom.config.get('jf-fuzzy-finder.preserveLastSearch')) {
  ...
  show () {
    ...
    this.panel.show()
    if (atom.config.get('jf-fuzzy-finder.prefillFromSelection') === true) {
